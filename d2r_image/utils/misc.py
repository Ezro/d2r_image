...
class WindowSpec:
    ...
    def match(self, hwnd) -> bool:
        ...
        if self.title_regex is not None:
            result = result and Regex(
                self.title_regex).matches(GetWindowText(hwnd))
        if self.process_name_regex is not None:
            _, process_id = GetWindowThreadProcessId(hwnd)
            result = result and Regex(self.process_name_regex).matches(
        ...
        for hwnd in window_list:
            if spec.match(hwnd):
                ...
                (left, top), (right, bottom) = ClientToScreen(
                    hwnd, (left, top)), ClientToScreen(hwnd, (right, bottom))
    ...
    if os.name == 'nt':
        ...
        for w in windows_list:
            if w[1] == "Diablo II: Resurrected":
                SetWindowPos(w[0], HWND_TOPMOST, 0, 0, 0,
...
def restore_d2r_window_visibility():
    if os.name == 'nt':
        ...
        for w in windows_list:
            if w[1] == "Diablo II: Resurrected":
                SetWindowPos(w[0], HWND_NOTOPMOST, 0, 0,
...
def trim_black(image):
    ...
    roi = np.min(x_nonzero), np.min(y_nonzero), np.max(x_nonzero) - \
        np.min(x_nonzero), np.max(y_nonzero) - np.min(y_nonzero)
    img = image[np.min(y_nonzero):np.max(y_nonzero),
    ...
    if os.path.isfile(path):
        try:
            template_img = cv2.imread(
                path, cv2.IMREAD_UNCHANGED) if alpha else cv2.imread(path)
            template_img = cv2.resize(
                template_img, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_NEAREST)
            return template_img
