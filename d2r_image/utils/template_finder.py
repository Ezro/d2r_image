...
import time
import os
from d2r_image.utils.misc import load_template, list_files_in_folder, alpha_to_mask, roi_center
from d2r_image.utils.screen import convert_screen_to_monitor, grab
import logging
...
class TemplateFinder:
    ...
    TEMPLATE_PATHS = [
        ...
        "templates\\npc",
        "templates\\shop",
        "templates\\item_properties",
        "templates\\chests",
        "templates\\gamble",
        "templates\\items_inventory"
    ]
    _instance = None
    ...
    def search_and_wait(
        ...
        """
        from d2r_image import UI_ROI
        if type(ref) is str:
            ref = [ref]
        start = time.time()
        while 1:
            img = grab()
            template_match = self.search(
                ref,
                img,
                roi=roi,
                threshold=threshold,
            ...
            is_loading_black_roi = np.average(
                img[:, 0:UI_ROI["loading_left_black"][2]]) < 1.0
            if not is_loading_black_roi or "LOADING" in ref:
                if template_match.valid:
                    logging.debug(
                        f"Found Match: {template_match.name} ({template_match.score*100:.1f}% confidence)")
                    return template_match
                if time_out is not None and (time.time() - start) > time_out:
                    logging.debug(f"Could not find any of the above templates")
                    return template_match
